...
function ensureMicOnGuideElement() {
  ...
  const button = controls?.querySelector("#shuffle-button");
  if (!controls || !button) {
    return null;
  }
  ...
  if (!participantGuide) {
    ...
    if (controlLine) {
      controls.insertBefore(summary, controlLine);
    } else {
      controls.insertBefore(summary, button);
    }
  ...
  if (!micOnGuide) {
    ...
    controls.insertBefore(guide, button);
    micOnGuide = guide;
  }
...
function getMaximumGroupCount(participantCount) {
  ...
  return Math.min(INPUT_COUNT, Math.floor(participantCount / MIN_MEMBERS_PER_GROUP));
}
...
function formatGroupSymbol(groupNumber) {
  ...
  return CIRCLED_GROUP_SYMBOLS[index] || String(groupNumber);
}
...
function renderGroups(groups) {
  const cards = groups
    .map((group, index) => {
      ...
      return `<article class="group-card result-pop">
        <h3>${groupPrefix}${facilitatorLabel}グループ</h3>
        <ul>${members}</ul>
      </article>`;
    })
    .join("");
  ...
  if (ensureModalElements()) {
    modalContent.innerHTML = `<div class="group-grid">${cards}</div>`;
  }
}
    ...
    if (heading) {
      ...
      const facilitatorLabel =
        state && state.facilitatorName !== "調整中"
          ...
          : "抽選中";
      heading.textContent = `${symbol}${facilitatorLabel}グループ`;
...
function renderDrawStage(_participants, groupCount) {
  ...
  const previewGroups = Array.from({ length: groupCount }, (_, index) => {
    ...
    return `<article class="group-card draw-group-card" data-preview-card>
      <h3 data-preview-heading="${index}">${symbol}抽選中グループ</h3>
  ...
  modalContent.innerHTML = `
    <section class="draw-stage-plain" aria-live="polite">
      <div class="draw-status-line">
        <span id="draw-phase" class="draw-phase-text">ファシリテーター決定中</span>
        <span class="draw-remaining">残り <strong id="draw-countdown">5.0</strong> 秒</span>
      </div>
      <div class="draw-progress-plain" aria-hidden="true"><span class="draw-progress-plain-bar"></span></div>
...
function runDrawAnimation(participants, groupCount, finalGroups) {
  ...
  const countdown = modalContent.querySelector("#draw-countdown");
  const phaseText = modalContent.querySelector("#draw-phase");
  ...
  const renderInitialFrame = () => {
    ...
    if (countdown) {
      countdown.textContent = (DRAW_DURATION_MS / 1000).toFixed(1);
    }

    if (phaseText) {
      phaseText.textContent = "ファシリテーター決定中";
    }
  };
...
async function handleShuffle() {
  ...
  if (requiredGroupCount > maximumGroupCount) {
    showError(
      `参加者${participants.length}人では、1グループ${MIN_MEMBERS_PER_GROUP}〜${MAX_MEMBERS_PER_GROUP}人で2グループ以上を作れません。参加者を${MIN_GROUP_COUNT * MIN_MEMBERS_PER_GROUP}人以上にしてください。`
    );
    return;
  }
  ...
  if (groupCount > maximumGroupCount) {
    showError(
      `1グループ最小${MIN_MEMBERS_PER_GROUP}人のため、参加者${participants.length}人は${maximumGroupCount}グループ以下を選択してください。`
    );
    return;
  }
  ...
  try {
    ...
    updateGroupSelectConstraints();
    shuffleButton.textContent = "振り分けスタート";
  }
}
